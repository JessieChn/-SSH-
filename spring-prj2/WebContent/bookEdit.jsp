...
<s:form  action="bookUpdate" method="post" namespace="/" theme="simple">
请输入书名<s:textfield name="book.name" label="请输入书名" onBlur="checkBookName()"  value="%{name}"/>
<s:label id="namespan" value="匹配书名是否合法(不为空且不超过12个字节)" ></s:label><br>
图书图片<s:file  name="upload" label="图书图片"/>  <br>
请输入图书的描述<s:textfield name="book.description"  label="请输入图书的描述"  onBlur="checkBookDescription()"  value="%{description}"/>
<s:label id="descriptionspan" value="匹配书名是否合法(不为空且不超过55个字节)"></s:label><br>
请输入图书的价钱<s:textfield name="book.price" label="请输入图书的价钱"  onBlur="checkBookPrice()"  value="%{price}"/>
<s:label id="pricespan" value="匹配书的价钱是否合法(1到6位且只为数字)"></s:label><br>
图书作者<s:textfield name="book.writer"  label="图书作者"  onBlur="checkBookWriter()"  value="%{writer}"/>
...
,'西安电子科技大学出版社':'西安电子科技大学出版社'}"  value="%{press}"  label="图书的出版社"/><br>
图书作者地域<s:select name="book.writerLocation" list="#{'中国':'中国','美国':'美国','欧印':'欧印','新马泰':'新马泰','非洲南美':'非洲南美'}" value="%{writerLocation}"  label="图书作者地域"/><br>
图书类别<s:select name="book.type" list="#{'文学综合馆':'文学综合馆','童书馆':'童书馆','教育馆':'教育馆',
'人文社科馆':'人文社科馆','经管综合馆':'经管综合馆','励志综合馆':'励志综合馆','生活馆':'生活馆','艺术馆':'艺术馆',
'科技馆':'科技馆','计算机馆':'计算机馆','杂志期刊馆':'杂志期刊馆'}" value="%{type}"  label="图书类别"/><br>
